...
Route::middleware(['token.decrypt', 'auth:sanctum', 'user-in-company', 'payload.decrypt'])->controller(RoleController::class)->prefix('companies/{company}')->group(function () {
    Route::get('/user-roles', 'fetchUserRoles')->name('fetch.user.roles')->withoutMiddleware(['token.decrypt','auth:sanctum', 'payload.decrypt','user-in-company']);
